...
public class OrderInputFrame extends JFrame implements ActionListener {
	...
	public OrderInputFrame(Customer customer) {
		...
		add(btnModify);
//----------------------------------------------------------------------
		btnHistory = new JButton("注文履歴確認");
		//setBounds(int x, int y, int width, int height)
		btnHistory.setBounds(350, 130, 120, 30);
...
//----------------------------追加コード-------------------------------------------------------------
			...
			try {
				...
				ArrayList<OrderDetail> orderDetailList = KiddaLaController.orderHistory(custId);

				if (orderDetailList.size() != 0) {

					setVisible(false);
					new HistoryFrame(orderDetailList);

				} else {

					JOptionPane.showMessageDialog(this, "注文履歴はありません。", "【確認】", JOptionPane.INFORMATION_MESSAGE);
				}
